  ...
  return (next) => (action) => {
    ...
    if (action.type == 'MESSAGES_SOCKET_INIT') {
      ...
      socket.on('get-users', (users) => {
        ...
        const workgroupMembers = store.getState().workgroups.currentWorkgroup?.members;
        users.forEach((user) => {
          if (
            Array.isArray(workgroupMembers) &&
            !workgroupMembers.find((wgMember) => wgMember._id == user.userId)
          )
            hasNewUser = true;
        });
        if (hasNewUser) store.dispatch(getWorkgroups(user.token));
    ...
    if (action.type == 'CREATE_CONVERSATION_SUCCESS') socket.emit('new-user-add', user._id);
