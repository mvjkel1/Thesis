  ...
  switch (type) {
    ...
    case 'MESSAGE_NOTIFICATION':
      return {
        ...
        isLoading: false,
        loaded: true,
        conversations: state.conversations.map((conv, i) =>
    ...
    case 'CLEAR_MESSAGE_NOTIFICATION':
      return {
        ...
        conversations: state.conversations.map((conv, i) =>
          conv._id == payload.conversationId
            ? { ...conv, unread: true, messages: [...conv.messages], notification: null }
            : conv
        )
      };
    case 'GET_MESSAGES_SUCCESS':
      return {
        ...
        loaded: true,
        conversations: state.conversations.map((conv, i) =>
          conv._id == payload[0]?.conversationId ? { ...conv, messages: payload } : conv
        )
      };
