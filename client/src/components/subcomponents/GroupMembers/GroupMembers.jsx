...
import { RecentlyActiveContainer, UserEntryContainer, StyledBadge } from './GroupMembers.styles';
...
const UserEntry = ({ user, currentUser, activeUsers, notifications, onChatStart }) => {
  const isOnline =
    activeUsers.some((usr) => usr.userId == user._id) && user._id !== currentUser._id;
  ...
  return (
    <UserEntryContainer>
      <Box sx={{ display: 'flex', alignItems: 'center' }}>
        ...
        {notifications.some((notif) => notif?.senderId == user._id) && (
          <FiberManualRecordIcon sx={{ height: 15, width: 15 }} color="error" />
        )}
        <IconButton disabled={isCurrentUser} onClick={() => onChatStart(user)}>
          <ChatBubbleIcon color={isCurrentUser ? 'neutral' : 'primary'} />
        </IconButton>
      </Box>
    </UserEntryContainer>
  );
...
export const GroupMembers = () => {
  ...
  const setTimeagoLocale = useMemo(() => register('my-locale', localeFunc), []);

  useEffect(() => {
    if (currentUser._id) dispatch(messagesSocketInit());
  }, [currentUser]);

  const onChatStart = (toUser) => {
    //check if there is an existing conversation between users
    const existingConversation = conversations.find((conv) =>
      conv.members.some((member) => member == toUser._id)
    );
    if (existingConversation) navigate(`/chat/${existingConversation._id}`);
    else
      dispatch(createConversation({ receiverId: toUser._id }, currentUser.token)).then((resp) =>
