...
import { Avatar, AvatarGroup, Box, Button, Chip, Collapse, IconButton } from "@mui/material";
...
export const Moodboard = () => {
    ...
    const drawPrevious = (data, canvas, context) => {
      var w = canvasRef.current.width;
      var h = canvasRef.current.height;
      data.forEach(entry => drawLine(entry.x0 * w, entry.y0 * h, entry.x1 * w, entry.y1 * h, entry.color, canvas, context))
    }
    ...
    return (
        <FeatureContainer>
          ...
          <Box mt={1} sx={{display: "flex", gap: 1, alignItems: "center"}}>
            ...
            <Box sx={{display: "flex", gap: 0.50}}>
              {["red", "black", "lightblue"]. map(color => <Chip label="" sx={{backgroundColor: color, "&&:hover": {backgroundColor: color}, width: 32}} onClick={() => board.current.color = color} variant="outlined" />)}
            </Box>
